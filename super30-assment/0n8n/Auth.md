...
It checks if a request is for a protectedRoutes (e.g., /dashboard). If the token cookie is missing, it redirects the user to /signin.

It checks if a request is for a publicRoutes (e.g., /signin, /signup). If the token cookie is present, it redirects the already logged-in user to the /dashboard, preventing them from seeing the sign-in page again.
...
User Submits Form: The user fills in the LoginPage form and submits.

Frontend API Call: The browser sends a POST request to http://localhost:8080/api/v0/user/signin. The token cookie is not present yet.

Backend Verification: Express verifies the credentials. On success, it generates a JWT and sends back a response with the Set-Cookie header containing the httpOnly token.

Browser Stores Cookie: The browser receives the response and securely stores the token cookie.

Frontend Redirects: The Axios request resolves successfully, and the Next.js router redirects the user to /dashboard.

Accessing a Protected Page

Browser Requests /dashboard: The browser now makes a GET request to the Next.js frontend for the /dashboard page. It automatically attaches the token cookie to this request.

Next.js Middleware Runs: The middleware sees the request is for /dashboard and confirms the token cookie exists. It allows the request to proceed.

Dashboard Layout Renders on Server: The DashboardLayout server component executes.

Server-to-Server API Call: The getUser function inside the layout reads the cookie from the initial request and makes a new GET request to the backend's /me endpoint, forwarding the cookie.

Backend Verifies Token: The protect middleware on the backend verifies the JWT and returns the user's data.

UI Rendered and Sent: The DashboardLayout receives the user data, renders the complete page HTML (including the "Welcome, {user.userName}!" message), and sends this HTML to the browser for display.
